...
def test_dbg_verify_reports_complete_current_format_matrix() -> None:
    ...
    assert "trace_schema_version=17" in result.output
    assert "replay_format_version=18" in result.output
    assert "checkpoint_format_version=5" in result.output
    assert "frida_capture_format_version=25" in result.output
...
def test_dbg_health_on_recorded_trace(tmp_path: Path) -> None:
    ...
    assert "trace_schema_version=17" in health_result.output
