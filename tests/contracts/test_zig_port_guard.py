...
def test_zig_bonus_ids_match_python_port() -> None:
    # "projectile_fork", "blade", "explosive_payload", "plasma_overload", and
    # "ion_overload" are Python-rewrite-only bonuses with no native reference
    # and no Zig port counterpart, same reason "maps" is excluded from the
    # game-mode-ids guard above.
    assert _zig_enum_values("BonusId") == _python_enum_values(
        BonusId,
        exclude={"projectile_fork", "blade", "explosive_payload", "plasma_overload", "ion_overload"},
    )
...
def test_zig_game_mode_ids_match_python_playable_modes() -> None:
    # "maps" is a Python-rewrite-only mode with no native reference and no Zig
    # port counterpart, same reason "demo" is already excluded here.
    assert _zig_enum_values("GameModeId") == _python_enum_values(GameMode, exclude={"demo", "maps"})
